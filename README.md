...
| 稼働率 39.9% | 56% |
| **段階2「月4件で年収2,010万」** | **成立しない**（1,834h 必要／使えるのは 1,373h） |
| 天井 年収 2,172万 | 約 1,950万（余白ゼロ） |
| 松竹梅＝尺 | 松竹梅＝仕上げの段階 |
| 32通りで採算を検証 | 102通り |
...
| 段 | 工数倍率（価格の前提） | 特典から積み上げた工数 |
|----|---------:|---------:|
| 梅 | ×1.0 | 12.8h |
| 竹 | ×1.4 | 14.9h（×1.17） |
| 松 | ×1.9 | 17.9h（×1.40） |

**この2列は一致していない。** 価格は「工数と同じ倍率をかければ時間単価が一定」
という前提で組んであるが、特典を絞った結果、工数の伸びは価格より緩くなった。
上の段ほど時間単価が上がる（¥23,156→¥26,925/h）ので実害はないが、
**いまの価格には原理的な裏づけがない**ことは認識しておく。

しかも右列の工数自体、**実測ではない**。実測1本の合計15hを工程別に配分した仮定で、
「キャラクターシート1人 = 0.75h」等に根拠はない。
`成果物/_テンプレート/工数記録シート.md` を次の案件で埋めて実数に差し替えること。

なお納期の計算にはいまも ×1.0/1.4/1.9 を使っている。実工数がこれより軽いなら
納期は長めに出るだけなので、実測が揃うまではこのままでよい。
差が出るのは3分以上だけで、60秒までは下限の2週間に張り付くため変わらない
（5分の松：モデル7週 → 実測ベースなら4週）。
...
| 検証 | 内容 |
|------|------|
| 時間単価が目標の95%を下回らない | 最低は松300秒×6本の ¥14,597（目標比98.0%） |
| 時間単価が現実離れしていない | 最高 ¥15,160。計算式が壊れたら跳ねるので検知できる |
| 上の段ほど時間単価が実質的に下がらない | 一時この並びが逆転しており、特典を組み直す理由になった |
| **工数モデルが実測より短くない** | 短い側に倒れると納期に遅れる。倍率を下げるなら実測を伴わせること |
...
- **段ごとの工数は未実測**。実測は「59秒で15時間未満／15秒で2時間」の2点だけで、
  しかも記憶ベース。工程別の内訳は仮定にすぎない。次の案件で記録シートを埋めること。
- **1件も受注していない**。価格が市場で通るかのデータがゼロ。とくに5分の松は
  ¥2,085,000 になるが、これが通るかは工数とは無関係の話。`funnel.html` で見る。
- **クレジットはプラン内に収まる見込み**。59秒の松1本で生成1,221。Max は月5,400なので
  月2件なら収まるが、撮り直しが増えた月は足りなくなる。
