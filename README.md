...
```
料金 = 基本料金 ¥90,000 ＋ 秒単価 × 合計秒数 ＋ ¥65,000 × (本数 − 1) ＋ ¥70,000 × ナレーター人数
工数 =        3.0h      ＋ 0.15h × 倍率 × 合計秒数 ＋ 1.5h × (本数 − 1) ＋ 1.0h × ナレーター人数
```

ナレーションは **1名 ¥70,000〜**。**尺でも本数でもなく「人数」で増える。**
1人が同じ収録で複数本を読んでも手配は1回で、費用もほぼ変わらない。
声を2人使うなら別途見積り（計算機は1名までしか選べない）。

2026-09-17 に「1本 ¥30,000」から変更した。**合成音声の日本語が使えなかったため。**
4エンジン（Seed Audio / MiniMax / ElevenLabs / Seed Speech）× 9声を試して全部不採用
（`成果物/サンプル映像/工数記録_サンプル映像03_05_07.md`）。
実案件では人のナレーターを手配する前提になるので、**¥70,000 の大半は外注費**。
だから「¥70,000 ÷ 1.0h」は自分の時間単価ではない。

**「〜」は下限**。声によって変わるので、計算機はナレーションありのとき合計にも「〜」を付ける。
...
|----|-------:|----------|--------------|-----:|----------------|
| 梅 標準 | ¥3,500 | なし（設備・製品中心） | 1名 ¥70,000〜 で追加可（基本はテロップ） | 2回 | SNS投稿・社内共有 |
| 竹 上 | ¥4,900 | **2人まで** | 1名 ¥70,000〜 で追加可（基本はテロップ） | 3回 | 採用サイト・会社紹介 |
| 松 特上 | ¥6,650 | 3人まで | **1名込み**（2人目は別途見積り） | 3回 | 展示会・ブランド映像 |
...
ナレーションは段によらず 1名 1.0h・¥70,000〜 で足す。松は1名ぶんが倍率に入っているので
加算しない。工数の 1.0h は自分の作業（原稿・声の選定・配置）だけで、
**ナレーターへの外注費は工数ではなく原価**。
...
| ナレーションを足しても全体の時間単価が目標を割らない | ¥70,000 は外注費を含むので「÷1.0h」を単価として使わない。全170通りの下限で見る。納期も +1週以内 |
| 松は1名ぶんが ¥0（込み）で、本数では増えない | 計算機の表示・mailto・FAQ の文言が同じ額・同じ単位（名）を指す |
