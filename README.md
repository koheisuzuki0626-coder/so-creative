...
工数 =        3.0h      ＋ 0.15h × 倍率 × 合計秒数 ＋ 1.5h × (本数 − 1)
```
...
|----|-------:|----------|--------------|-----:|----------------|
| 梅 標準 | ¥3,500 | なし（設備・製品中心） | なし（テロップ） | 2回 | SNS投稿・社内共有 |
| 竹 上 | ¥4,900 | **2人まで** | なし（テロップ） | 3回 | 採用サイト・会社紹介 |
| 松 特上 | ¥6,650 | 3人まで | **あり**（声と話す速さを調整） | 5回 | 展示会・ブランド映像 |

画質は全段 1080p で共通。**1段上がるごとに増えるものを1つに絞ってある**
（梅→竹は「人が出る」、竹→松は「声が入る」）。お客様が迷うのはこの2点だけ。

**差をつけるのをやめたもの**（2026-09-14 の実証による）

| 落としたもの | 理由 |
|--------------|------|
| 画質 4K 対応 | 1080p からの引き伸ばし。原価1.2クレジット＝処理の中身が薄い。納品先は 1080p で足りる |
| 横型・縦型・正方形の3形式 | リフレームはテロップを組み直さないので、縦型が使いものにならない |
| 高精細化・ちらつき除去 | 4K化と同じ処理に含まれ、単独では意味がない |
...
| 竹 | ×1.25 | 8.9h | 梅 ＋ キャラクターシート1.0 ＋ 修正1回0.6 |
| 松 | ×1.55 | 11.1h | 竹 ＋ ナレーション1.0 ＋ 修正2回1.25 |

モデル（3.0h ＋ 0.15h × 倍率 × 秒）はこの 1.5〜1.6倍。60秒の松で 17.0h（実測 10.5h）。
以前の 4.8h ＋ 0.24h × 1.0/1.4/1.9 は実測の **3.1倍**で、納期が長く出すぎていた
（5分の松で7週）。価格の倍率 1.0/1.4/1.9 は据え置いたので、時間単価は
梅 ¥26,852 → 竹 ¥28,465 → 松 ¥30,072/h（102通りの平均）と上の段ほど高い。
...
| 時間単価が現実離れしていない | 最高 ¥32,526。計算式が壊れたら跳ねるので検知できる |
...
秒単価の倍率（1.0/1.4/1.9）が工数の倍率（1.0/1.25/1.55）以上なので、
段が上がっても時間単価は下がらない。
全102通りをクリックして検証しており、**¥23,750〜32,526/h** に収まる。
