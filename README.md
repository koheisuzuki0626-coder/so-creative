...
```
料金 = 基本料金 ¥90,000 ＋ 秒単価 × 合計秒数 ＋ ¥65,000 × (本数 − 1) ＋ ナレーション
工数 =        3.0h      ＋ 0.15h × 倍率 × 合計秒数 ＋ 1.5h × (本数 − 1) ＋ 1.0h × ナレーション本数

ナレーション ／ AI音声 ¥30,000 × 本数 ｜ 人が読む ¥70,000〜 × 1名（本数では増えない）
```

#### ナレーションは AI音声と人が読むの2択（2026-09-17）

| | 料金 | 単位 | 中身 |
|---|---:|---|---|
| AI音声 | ¥30,000 | **1本** | 声の選定・速さの調整・配置。**機械の声だと分かる仕上がり** |
| 人が読む | ¥70,000〜 | **1名** | 上記＋ナレーターの手配。1人が複数本読んでも手配は1回 |

**単位が違う。**AI は原稿・配置が本数ぶん要るので本数に比例する。
人は手配が1回なので本数では増えない（声を2人使うなら別途見積り）。
**工数はどちらも 1本 1.0h。**人の ¥70,000 の大半はナレーターへの外注費で、
...
そこで人のナレーターを足し、AI音声は**「機械の声だと分かる」と明記して**残した。
社内向けや、テロップ主体で声を補助的に使う用途なら成立するため。

**「〜」は下限。**人が読む場合は声によって変わるので、計算機は合計にも「〜」を付ける。
AI音声は額が決まっているので付けない。
...
|----|-------:|----------|--------------|-----:|----------------|
| 梅 標準 | ¥3,500 | なし（設備・製品中心） | AI音声 1本 ¥30,000／人 1名 ¥70,000〜（基本はテロップ） | 2回 | SNS投稿・社内共有 |
| 竹 上 | ¥4,900 | **2人まで** | AI音声 1本 ¥30,000／人 1名 ¥70,000〜（基本はテロップ） | 3回 | 採用サイト・会社紹介 |
| 松 特上 | ¥6,650 | 3人まで | **人が読む1名が込み**（2人目は別途見積り） | 3回 | 展示会・ブランド映像 |
...
| ナレーションを足しても全体の時間単価が目標を割らない | 人の ¥70,000 は外注費を含むので「÷1.0h」を単価として使わない。AI・人それぞれの下限で見る。納期も +1週以内 |
| 松は人が読む1名ぶんが ¥0（込み）で、本数では増えない | 計算機の表示・mailto・FAQ の文言が同じ額・同じ単位を指す |
| **「竹＋ナレーション」が松より高くなるときは、その場で「松のほうが安い」と出す** | 松は竹＋ナレの上位互換なので、黙って逆転させると下位の仕様を高く買う選択肢ができる。短い尺では松の秒単価に溶けているナレーション相当（竹との差 1,750/秒）が足す額に届かず逆転するため、価格ではなく表示で潰す |
...
段が上がっても時間単価は下がらない。
全238通り（段×尺×本数×ナレーション3択）をクリックして検証しており、**¥23,750〜34,618/h** に収まる。
