...
ナレーションの項は梅・竹だけ。松は秒単価に含まれている（倍率 1.36 の中に 1.0h が入っている）。
...
| 松 特上 | ¥6,650 | 3人まで | **込み**（声と話す速さを調整） | 3回 | 展示会・ブランド映像 |

画質は全段 1080p で共通、4K（高精細化）での納品も全段で追加費用なし。
**1段上がるごとに増えるものを1つに絞ってある**（梅→竹は「人が出る」、竹→松は「声が込みになる」）。
...
梅・竹にナレーションを足すときは 1本 1.0h を別に足す（松の倍率に入っているのと同じ量）。
...
梅 ¥26,984 → 竹 ¥29,002 → 松 ¥33,262/h（170通りの平均）と上の段ほど高い。
...
| 時間単価が現実離れしていない | 最高 ¥35,027。計算式が壊れたら跳ねるので検知できる |
...
| 松では ¥0（込み）、梅・竹では本数ぶん加算 | 計算機の表示・mailto・FAQ の文言が同じ額を指す |
...
段が上がっても時間単価は下がらない。
全170通り（段×尺×本数×ナレーション有無）をクリックして検証しており、**¥23,750〜35,027/h** に収まる。
