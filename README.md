...
画質は全段 1080p で共通。4K は**売り文句にしない**（1080pからの引き伸ばしで情報が増えないため）。
FAQ に1問だけ置き、求められたときは追加費用なしで対応すると正直に書いてある。
...
| 落としたもの | 理由 |
|--------------|------|
| 画質 4K 対応 | 1080p からの引き伸ばし。原価1.2クレジット＝処理の中身が薄く、納品先（採用サイト・55インチ）は1080pで足りる。**売り文句から外し、FAQ で「対応はするが中身は1080p」と書く**（2026-09-16。一度「全段で無料」と特典に戻したが、確かめられない違いを売り文句にするのは同じ誤り） |
