...
# 2026-09-21 に書き直した。前は映画学校の設問（演出・カメラワーク・VFX…）で、
# 返ってくるのが「【】で煽る」「数字を入れる」のような一般論だった。
# こちらが必要なのは【明日そのまま真似できる粒度】と【AIで作れるかの判定】。
# 撮影しない前提なので、実写でしか撮れない画を褒められても使えない。
VIDEO_STUDY_PROMPT = (
    "あなたはAI映像制作者。撮影は行わず、生成AI（Seedance・Klingなど5〜15秒の"
    "生成をつないで作る）で同等の映像を作れるかという目で、この動画を視聴して。\n"
    "推測では書かず、実際に見えたものだけを書く。分からない項目は「不明」と書く。\n\n"
    "① 冒頭3秒：何が映っていて、何を言っているか（具体的に）\n"
    "② カット割り：カット数と1カットの平均秒数。**数えた値か推定かを必ず書く**\n"
    "　　（例「42カット・数えた」「約80カット・推定」）。数えられないなら「不明」\n"
...
_TREND_STRIP_TAIL_RE = re.compile(
    ...
    # 操作の言葉。題材ではない（2026-09-10）。
    # 事故：「YouTubeリサーチ実行して」の『実行』が検索語になり、
    # Snow Man・サッカー・HoneyWorks を分析していた。
    r"実行して|実行|開始して|開始|スタートして|スタート|"
    r"start|go|走らせて|動かして|"
...
_TREND_FRAME_RE = re.compile(
    ...
    # 「回す」はこのボットでリサーチを走らせる時の口癖。ボット自身が
    # 「『企業VPで回して』と送ってください」と案内するのに、この語が
    # 検索語の取り出しにも起動の判定にも入っていなかった（2026-09-04）。
    r"(検索|調べ|リサーチ|探し|探す|回し|回す)", re.S)
...
# 題材として成立しない語の一覧（最後の関門）。
# 「今」「実行」「他の動画ない」…と、同じ壊れ方を4回繰り返した（09-04/08/09/10）。
# 削り方を足すだけでは、次の言い方でまた漏れる。取り出した【結果】を見て、
# 操作・時・指示語だけの短い語なら題材として認めない。
# 語を数え上げているが、ここは「削り漏れの最後の受け皿」なので、
# 漏れた時の被害（無関係な動画を50本分析する）に対して割に合う。
_NOT_A_TOPIC_RE = re.compile(
    r"^(実行|開始|スタート|start|go|やる|やって|再開|続行|続き|"
    r"今|いま|今すぐ|すぐ|次|前|それ|これ|あれ|そこ|ここ|"
    # 「クロード1、以下の事例を探して」の『以下』（2026-09-12）
    r"以下|下記|上記|例|事例|参考|作例|サンプル|"
    r"もう一回|もう一度|再度|また|他|ほか|別|全部|ぜんぶ|すべて|"
    r"リサーチ|調査|トレンド|急上昇|youtube|ユーチューブ)$", re.I)


def _trend_topic(text):
    """リサーチの依頼文から、実際に検索する語だけを取り出す。
    取り出せなければ空（＝前回の題材・毎朝のお題・急上昇TOP100の順で決まる）。"""
    if _TREND_MORE_RE.match((text or "").strip()):
        return ""                          # 続きの依頼。呼び出し側が前回の題材を使う
    t = _strip_media_context(_pick_trend_line(text) or "").strip()
    # 頭の呼びかけ（「クロード1、」「リサーチャー、」）は題材ではない。
    # 事故（2026-09-12）：「クロード1 会社紹介動画の事例探して」が
    # 『クロード1 会社紹介動画の事例探』で検索された。
    t = re.sub(r"^(クロード\s*[0-9０-９]?|クロード１|リサーチャー|PM|アドバイザー)"
               r"\s*[、,：:]?\s*", "", t).strip("　 。、")
    # 「〜の事例／参考／作例を探して」の枠も落とす（題材はその前）
    t = re.sub(r"\s*(の)?(事例|参考|作例|サンプル)(動画)?[をも]?\s*"
               r"(探して|さがして|検索して|集めて|見つけて|拾って)?\s*$",
               "", t).strip("　 。、")
    ...
    if _TREND_NOT_TOPIC_RE.search(t):      # 依頼の訂正＝検索語ではない
        return ""
    # 最後の関門：削り残しが操作・時・指示語だけなら、題材として認めない。
    # ここが無いと、削り方の漏れがそのままYouTubeの検索語になる。
    if _NOT_A_TOPIC_RE.match(t):
        return ""
    return t
...
def _past_items(key, limit=25):
    ...
    末尾N字を渡す作りだと直近1〜2件しか入らず、同じ結論が何度も出た
    （2026-09-22：「背景単体は生成できる」「文字・ロゴは作れない」が
...
def _recent_insights(limit=1500):
    """これまでに貯めた知見の末尾を返す（同じ結論を毎回書かせないために渡す）。"""
    try:
        path = NOTES["insight"][0]
...
async def _run_trend_study(cid, query=None, skip_analyzed=None,
    ...
    if reports:
        ...
        digest_prompt = (
            ...
            "書いて、新しく分かったことを1行足す\n"
            "・元の分析が「不明」としている数字を、こちらで埋めない\n\n"
            + MY_PRICING_NOTE + "\n\n"
            + ("【既出：次に試すこと（新しい順。同じ提案は禁止）】\n"
...
#
# 【助詞を必須にしてある】理由（2026-08-31の事故）：
# 「YouTubeリサーチもう少し短く簡潔にまとめ直して」＝出来上がりの【出し方】への
# 注文が、ジャンル指定として読まれ、毎日のリサーチのお題が
# 「もう少し短く簡潔にまとめ直」に書き換わった（翌朝それでYouTubeを検索しに行く）。
# 「リサーチ」の直後に助詞（は/を/、）も「のジャンル」も無い文は、
# お題の指定ではなく本文への注文。文法の形で落とす（語を数え上げない）。
...
# 助詞があっても【お題ではなく注文】のことがある（「リサーチをもう少し短くして」）。
# お題は名詞。程度の副詞で始まる／連用形で終わるものは「どう出すか」の注文。
_TREND_GENRE_NG_RE = re.compile(
    r"^(?:もう少し|もうすこし|もっと|少し|すこし|ちょっと|やや|"
    r"なるべく|できるだけ|なるたけ)"
    r"|(?:短く|長く|詳しく|細かく|簡潔|丁寧|分かりやすく|わかりやすく|"
...
def _match_trend_genre(text):
    ...
    if not on_topic:
        return None
    # 【今回だけ回して】という依頼を、毎日の設定の変更にしない。
    # 事故（2026-09-04 16:53）：「YouTubeリサーチを企業VPに関連する語句で回して」
    # が毎日のお題の変更として扱われ、翌朝から「企業VPに関連する語句で回」
    # という壊れた語で検索する設定になった。
    # 原因は「回【して】」が、設定変更の「〜にして」の“して”に一致したこと。
    # 設定の変更は「〜にして／に絞って／に変えて」、今回だけの依頼は
    # 「〜で回して／で検索して／で調べて」。見分けるのは動詞。
    if re.search(r"(回して|回す|まわして|検索して|調べて|探して|"
                 r"見てきて|やって)\s*[。、!！]?$", t):
        return None
    ...
    # お題は _trend_topic と同じ整え方を通す。ここを通さないと
    # 「企業VPに関連する語句で回」のような言い回しがそのまま設定に残る。
    genre = _trend_topic(genre) or genre
    ...
    if not genre or _parse_jst_hour(genre) or re.search(
        ...
        return None
    # お題ではなく「どう出すか」への注文（「もう少し短く簡潔にまとめ直して」）は、
...
class RouteCtx:
    ...
    def __init__(self, content, *, has_attachments=False, has_video_att=False,
        ...
        # さらに、本人が貼った【引用】も落とす。
        # 事故（2026-09-12）：「クロード1、以下の事例を探して」の後にボットの
        # 過去の返事を丸ごと貼ったところ、引用内の「TOPPANの実績紹介動画」の
        # 『実績』で、自分のチャンネルの実績分析が始まった。
        # 指示は頭の1行で、残りは参考として貼っただけだった。
        self.raw_text = content or ""
        self.text = _instruction_part(self.raw_text)
        self.cid = cid
...
# 文章の成果物（リサーチ結果・知見・要約）への「言い直して」は、動画や画像の
# 作り直しではない。事故（2026-08-31）：「YouTubeリサーチもう少し短く簡潔に
# まとめ直して」が、まずお題の設定を書き換え（修正済み）、その先では
# 【作り直し】＝直前に作った動画の再生成へ流れていた。素材が付いていない
# 文章への注文は、絵や動画に触らせない。
_TEXT_ARTIFACT_RE = re.compile(r"リサーチ|知見|要約|レポート|分析結果|まとめ(?!て)")
_REWORD_RE = re.compile(
    r"短く|長く|簡潔|詳しく|細かく|分かりやすく|わかりやすく|読みやすく|"
    r"端的|まとめ直|要約|書き直|言い直")


def _r_text_rework(c):
    if c.has_attachments or c.has_image_att or c.has_video_att:
        return None                      # 素材を渡されているなら別の話
    if _TEXT_ARTIFACT_RE.search(c.text) and _REWORD_RE.search(c.text):
        return STOP_CHAT                 # 会話として答える（作り直しに流さない）
    return None
...
_R1_SEARCH_RE = re.compile(
    ...
    r"|(事例|参考|作例|サンプル動画)[^。\n]{0,14}"
    r"(探して|さがして|検索して|集めて|見つけて)")


def _r_youtube_research(c):
    if c.is_question:
        return None
    if _R1_SEARCH_RE.search(c.text):
        return "trend"
    if _YT_RESEARCH_RE.search(c.text):
        return STOP_CHAT                 # AI判定（trend）へ回す
    # 「企業VPで回して」＝直前のリサーチの話を受けた起動の合図。
    # 事故（2026-09-04 16:52）：ボット自身が「『企業VPで回して』と
    # もう一度送ってください」と案内したのに、その言い方が語彙に無く
    # 会話に落ちて「まだ何も動かしていないよ」で終わった。
    # 「回して」だけでは何の話か分からないので、直前がリサーチの話の時だけ通す。
    if re.search(r"[でを]\s*(回して|まわして|回してみて)", c.text):
        prev = _recent_bot_say(c.cid) or ""
        if (re.search(r"リサーチ|検索|動画|急上昇|回す|回し", prev)
                or _last_trend_topic.get(c.cid)):
            return "trend"
    return None
...
def _r_generate(c):
    ...
    if re.search("モーション|この動き|動きを", c.text):
        return None
    # 【文字】を書いてほしい依頼を、絵の生成に流さない。
    # 事故（2026-09-02）：NotebookLMの入力欄のスクショを添付して
    # 「ここの文章を考えて欲しい」と送ったら、画像添付だけを根拠に生成へ流れ、
    # 9項目の確認を出したうえでクレジットを消費する生成が始まった。
    # 添付は「これで何かして」の意思表示だが、【何をしてほしいか】は本文にある。
    # 求められているのが文章なら、その添付は素材ではなく“読む対象”。
    if _WANTS_TEXT_RE.search(c.text):
        return None
...
def _r_slideshow(c):
    ...
    if c.is_question:
        return None
    # 求められているのが【文字】なら動画化に流さない。_r_generate と同じ守り。
    # 「どんな動画にも使えるプロンプトにして」は、文中に「動画」があり
    # 依頼の形でもあるので、画像を添付していると動画化に取られていた。
    if _WANTS_TEXT_RE.search(c.text):
        return None
...
_PROGRESS_START_RE = re.compile(
    ...
    "投げる|投入する|流しておく|"
    # 事故（2026-09-04 10:08）：「じゃあ『採用動画 中小企業』で回します」と
    # 言ったきり一度も実行せず、6時間後に「まだ結果が返ってきていません」と
    # 答えた。ユーザーは存在しないジョブを待ち続けた。
    # 「回す」はこのボットでリサーチ・解析を走らせる時の口癖。
    "回します|回すね|回すよ|回して(る|いる|おく|みる)|回しとく|"
    "かけておく|かけときます|走らせる|"
...
_NOTIFY_LATER_RE = re.compile(
    ...
    "完了したら|出来たら|"
    # 事故（2026-09-04）：「結果が返ってきたら共有します」。
    # 走っていないのだから、返ってくるものは無い。
    "返ってきたら|戻ってきたら|上がってきたら|出たら|届いたら)"
    "[^。\n]{0,30}(知らせ|教え|連絡|報告|送|共有|貼)"
)
# 走っていないのに「結果待ち」であるかのように言う文。
# 事故（2026-09-04 16:43）：6時間前に「回します」と言ったきり実行しておらず、
# 「結果は？」に対して「まだ結果が返ってきていません」と答えた。
# 待っている対象が存在しないので、これは進捗ではなく作り話。
_FAKE_WAIT_RE = re.compile(
    "(結果|レポート|まとめ|分析)[^。\n]{0,14}"
    "(まだ|返ってきて|戻ってきて|上がってきて)[^。\n]{0,10}"
    "(ない|いません|きていません|きてない)|"
    "(まだ|いまだ)[^。\n]{0,12}(返ってきて|戻ってきて|届いて)[^。\n]{0,8}"
    "(ない|いません)|"
    "(結果|完了)待ち|待つ形になります|待ちの状態"
)

# ボット自身の進捗表示の語彙（「⏳ 続行中（N秒経過／実測M回では…／残り最大…）」）
# を、返事の中で真似て言うことがある。実際にはタスクは動いておらず
# （承認待ちのまま）、経過秒も残り時間も作り話。
# 事故（2026-08-29）：承認前の計画に対して「いま裏でエージェント実行が
# 回っていて、経過177秒」「エージェント実行が266秒経過」「残り最大5分36秒」
# と、走っているかのように答えた。_busy_tasks が空なら、これらは全部でたらめ。
_FAKE_PROGRESS_RE = re.compile(
    r"\d+\s*秒経過|経過\s*(は\s*)?\d+\s*秒|続行中|"
    r"残り\s*(最大|およそ|時間)|あと\s*最大\s*\d|"
    r"(裏|バックグラウンド)(で|グラウンドで)?[^。\n]{0,20}"
    r"(回って|動いて|走って|実行中|処理中)|"
    r"(エージェント実行|解析|処理|生成|変換|スキャン)[^。\n]{0,12}"
    r"(が|は)[^。\n]{0,12}(回って|動いて|走って|\d+\s*秒)|"
    # 「動いてる？」への「はい、動いています」単体の返し（走っていないので嘘）
    r"^(はい|ええ|うん)?[、。\s]*(いま|もう)?[、。\s]*"
    r"(動いて|回って|走って)(います|いる|る|ます)[。\s]*$"
)
...
def _drop_false_progress(text, cid):
    ...
    # 「N秒経過／続行中／裏で回っている」は、承認待ちの有無に関係なく、
    # _busy_tasks が空（ここに来ている時点でそう）なら作り話。
    kept = [p for p in parts
            if not (_PROGRESS_START_RE.search(p) or _NOTIFY_LATER_RE.search(p)
                    or _FAKE_PROGRESS_RE.search(p) or _FAKE_WAIT_RE.search(p)
