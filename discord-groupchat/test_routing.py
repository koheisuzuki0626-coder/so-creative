...
ROUTE_CASES = [
    ...
    # 元データHDDの動画をまとめて見る（自然文でも !analyze と同じ専用スクリプトへ。
    # エージェント実行は100本超でハングするため通さない）
    ("動画分析して", "hdd_analyze", {}),
    ("元データの動画を分析して", "hdd_analyze", {}),
    ("動画の解析進めて、同じ動画は飛ばしていい", "hdd_analyze", {}),
    ("HDDの動画を1本ずつ見て画の傾向をまとめて", "hdd_analyze", {}),
    ("動画分析って難しい？", "plan", {}),            # 質問は会話へ
    ("動画の分析結果どう思う？", "plan", {}),         # 感想を聞く質問は会話へ
    # 「解析どこまで？」は PM に推測で答えさせず台帳を数える専用ルートへ
    ("解析どこまで進んだ？", "hdd_progress", {}),
    ("分析何本終わった？", "hdd_progress", {}),
    ("元データの解析の進捗は？", "hdd_progress", {}),
...
def run():
    ...
    print("■ 「ローテーション」が画像モデルの一覧を誤爆させないこと")
    # 事故（2026-09-01）：リサーチの検索ワードを列挙して「ここら辺を
    # ローテーションしたい」と頼んだら、Gemini画像モデルの一覧が返ってきた。
    # コードには『この言葉はこの機能のことしか指さない』と書いてあったが、
    # その前提が崩れていた。何を回すかが書かれていれば従う／
    # 書かれていなければ【状態を尋ねる形】の時だけ状態を出す。
    for _t in ("ローテーションできてるのか分からない",
               "画像モデルのローテーションどうなってる？",
               "モデルのローテーション状況は？",
               "ローテーション回ってる？"):
        check(f"状態を出す: {_t!r}", bot._asks_image_model_status(_t), True)
    for _t in ("ここら辺をローテーションしたい",
               "検索ワードをローテーションして",
               "リサーチのテーマをローテーションしたい",
               "撮影のネタをローテーションしよう",
               "ローテーション組みたい",
               "画像生成のクレジットどうなってる"):
        check(f"状態を出さない: {_t!r}", bot._asks_image_model_status(_t), False)
    ...
    # 方針を変えた（2026-09-08）。以前は「全部外れたら外さずに見る」で
    # 0本を避けていたが、それだとフィルタが一番必要な場面で黙って無効になる。
    # 実際に「企業VP」15本が全部営業素材だった日、この逃げ道が働いて
    # BGM作品集を分析していた。いまは、そう言って止めて検索語の変更を促す。
    check("全部営業素材なら、黙って見ずに報告して止める",
    ...
    # 事故（2026-08-31）：「YouTubeリサーチもう少し短く簡潔にまとめ直して」＝
    # 出来上がりの【出し方】への注文が、お題の指定として読まれ、毎日のリサーチが
    # 「もう少し短く簡潔にまとめ直」でYouTubeを検索する設定に書き換わった。
    # お題は名詞。程度の副詞で始まる／連用形で終わるものは通さない。
    for _t in ("YouTubeリサーチもう少し短く簡潔にまとめ直して",
               "リサーチをもう少し短くして",
               "リサーチをわかりやすくまとめ直して",
               "YouTubeリサーチを詳しくして",
               "リサーチの結果を要約して"):
        check(f"出し方への注文はお題にしない: {_t[:24]!r}",
              bot._match_trend_genre(_t), None)
    # 2テーマの日替わり指定は、これまでどおり通ること（守りすぎの検出）
    check("日替わり2テーマは通る",
          bot._match_trend_genre(
    ...
    # AIが行ごと『』で囲んでくることがある（2026-09-23 16:50 のリサーチで実際に出た）。
    # こちらで計算して差し込んだ金額まで引用に見えてしまう。
    check("見積りの行の囲みは外す",
          "『" not in bot._attach_quote("『見積り条件: 尺=30秒 階層=梅』"), True)
    check("囲みを外しても計算は入る",
          "195,000" in bot._attach_quote("『見積り条件: 尺=30秒 階層=梅』"), True)
    check("本文の『』には触らない",
    ...
    print("■ リサーチの言い方が読めない時は、勝手に決めず聞き返すこと")
    ...
    check("この言い方をジャンル指定として読める",
          bot._match_trend_genre(
              "毎朝8時の自動リサーチを、日替わりで AI動画生成 と "
              "ミュージックビデオ の2テーマに切り替えて"),
          ("set", "AI動画生成 と ミュージックビデオ"))
    check("「日替わりで」は題材に混ぜない",
    ...
    check("時刻の指定はジャンルにしない",
          bot._match_trend_genre("毎日9時半にして"), None)
    check("停止の指定はジャンルにしない",
          bot._match_trend_genre("毎日のリサーチやめて"), None)
    ...
    for _t in ("今、試しにリサーチしてみて", "ちょっと今からリサーチして",
               "もう一度ちょっとリサーチして", "今すぐ試しに調査して"):
        check(f"重ねた副詞は題材にしない: {_t}", bot._trend_topic(_t), "")
    # 守りすぎの確認：副詞の後ろに題材があれば、それは残す
    for _t in ("今、ミュージックビデオをリサーチして",
               "ちょっとAI動画生成でリサーチして"):
        _got = bot._trend_topic(_t)
        check(f"副詞の後ろの題材は残す: {_t} → {_got}", bool(_got), True)
    ...
    # しか落としておらず、道具の名前（YouTube）と助詞が題材に混ざっていた。
    for _t, _want in (
        ("ミュージックビデオでYouTubeリサーチして", "ミュージックビデオ"),
        ("ミュージックビデオのトレンド調べて", "ミュージックビデオ"),
        ("MVのリサーチして", "MV"),
        ("かっこいいショート動画を調べて", "かっこいいショート動画"),
        ("広告動画の分析して", "広告動画"),
        ("fatboyslimのMVリサーチして", "fatboyslimのMV"),
        ("K-POPのミュージックビデオについて調べて", "K-POPのミュージックビデオ"),
        # 題材が無い＝急上昇TOP100を見る（道具の名前だけ残さない）
        ("YouTubeリサーチして", ""),
        ("YouTubeで急上昇調べて", ""),
        ("トレンド調べて", ""),
        # 事故（2026-09-01）：やり直しの合図が題材に混ざり、
        # 『もう一回会社プロモーション』という語で検索していた。
        ("もう一回会社プロモーションでYouTubeリサーチして", "会社プロモーション"),
        ("もう一度MVのリサーチして", "MV"),
        ("また会社紹介で調べて", "会社紹介"),
        # 事故（2026-09-01）：確認待ち中の【依頼の訂正】が、そのまま検索語に
        # なって「合う動画が見つかりませんでした」と返った。題材は名詞。
        ("編集はしなくていいよ、分析だけ", ""),
        ("動画生成じゃなくて分析して", ""),
        # 事故（2026-09-04）：回数の指示と題材が別の行に分かれて送られ、
        # 発言まるごとが検索語になって0件だった。「〜で検索」という
        # 一番普通の言い方が、末尾の依頼表現を削る方式では残っていた。
        ("YouTubeリサーチ今日4回やろ\n全部企業プロモーションビデオとかの語句で検索して",
         "企業プロモーションビデオ"),
        ("企業プロモーションビデオとかの語句で検索して", "企業プロモーションビデオ"),
        ("全部企業プロモーションビデオで検索して", "企業プロモーションビデオ"),
        ("会社紹介で検索して", "会社紹介"),
        ("採用動画というワードで調べて", "採用動画"),
        # 題材そのものに「で」が入る場合は、最後の「で検索」で切ること
        ("ドローンで撮影した映像で検索して", "ドローンで撮影した映像"),
        # 事故（2026-09-04 09:38）：リサーチ直後の「他の動画ない？」が
        # 『他の動画ない』という語で検索され、無関係な動画8本を取ってきた。
        # 続きの依頼なので題材は空にし、呼び出し側が前回の題材を使い回す。
        ("他の動画ない？", ""),
        ("他には？", ""),
        ("もっと見せて", ""),
        ("別の動画ある？", ""),
        # 守りすぎの確認：「他の〇〇で検索」は題材が書かれているので残すこと
        ("他の会社の動画で検索して", "他の会社の動画"),
        ("別の切り口で調べて", "別の切り口"),
        # 事故（2026-09-08 08:42）：「今リサーチしてみて」の『今』が検索語になり、
        # YouTubeで「今」を検索して50本取ってきた。時や様子を表す語は題材でない。
        ("今リサーチしてみて", ""),
        ("いまリサーチして", ""),
        ("今すぐリサーチして", ""),
        ("ちょっとリサーチして", ""),
        ...
        # 事故（2026-09-10 06:43）：「YouTubeリサーチ実行して」の『実行』が
        # 検索語になり、Snow Man・サッカー・HoneyWorks を分析していた。
        ("YouTubeリサーチ実行して", ""),
        ("リサーチ実行して", ""),
        ("リサーチ開始して", ""),
        ("リサーチをスタートして", ""),
        ("YouTubeリサーチ実行", ""),
        # 最後の関門：削り残しが操作・時・指示語だけなら題材にしない
        ("実行", ""), ("開始", ""), ("今", ""), ("それ", ""),
        ("もう一回", ""), ("他", ""), ("全部", ""), ("リサーチ", ""),
        # 守りすぎの確認：題材に「実行」「他」が含まれていても残すこと
        ("実行力のある会社", "実行力のある会社"),
        ("他の会社の動画で検索して", "他の会社の動画"),
    ):
        check(f"検索語を取り出す: {_t[:22]!r}", bot._trend_topic(_t), _want)

    print("■ 検索語の提案に合意したらリサーチが始まる（会話で終わらせない）")
    ...
    _prop = (
        "ここ数回「企業PV」系はどれも母数1〜2本です。検索語をずらした方が早い。\n"
        "- 「採用動画 中小企業」\n"
        "- 「ブランドムービー メーカー」\n"
        "- 「サービス紹介 アニメーション」\n"
        "どれで回すか一つ決めてくれれば、その語でリサーチします。")
    # 事故（2026-09-08）：「『企業VP』を外して、『採用ムービー』…で当てる」の
    # “外して”と言われた語が第1候補になっていた。
    check("外せと言われた語は候補にしない",
          bot._proposed_research_topics(
              "**検索語**は「企業VP」を外して、「採用ムービー」「ブランドフィルム」"
              "「周年記念ムービー」で当てる。リサーチの語はこの方向で。"),
          ["採用ムービー", "ブランドフィルム", "周年記念ムービー"])
    check("候補を箇条書きから取り出す（地の文の却下語は入れない）",
          bot._proposed_research_topics(_prop),
          ["採用動画 中小企業", "ブランドムービー メーカー",
           "サービス紹介 アニメーション"])
    _keep_say = bot._recent_bot_say
    try:
        bot._recent_bot_say = lambda cid: _prop
        for _t in ("おまかせする", "おまかせ", "ok", "ブランドムービー メーカーで"):
            check(f"提案への合意でリサーチへ: {_t}",
                  bot.classify_route(_t, cid=4242), "trend")
        # 守りすぎの確認：提案と関係のない発言は会話のまま
        for _t in ("しんどい", "ありがとう", "眠い"):
            check(f"関係ない発言は会話のまま: {_t}",
                  bot.classify_route(_t, cid=4242), None)
        # 直前がリサーチの提案でなければ「おまかせ」で始めない
        bot._recent_bot_say = lambda cid: "今日は暑いですね"
        check("提案が無ければ『おまかせ』で始めない",
              bot.classify_route("おまかせする", cid=4242), None)
    finally:
        bot._recent_bot_say = _keep_say

    print("■ 制作事例でない動画を弾く（BGM見本・ポートフォリオ・教材・サンプル）")
    # 本人の依頼（2026-09-06）：「企業VP」で検索すると、実制作の事例に混じって
    # 営業・見本・教材が上位に来る。プロモーションビデオになっていないものは弾く。
    # 実例は 09-05 のリサーチで実際に混ざったもの。
    for _t, _want in (
            ("【Commercial Music Reel】映像・CM・企業VP向けBGM作品集", "BGM・音源の見本"),
            ("企業VPサンプル（音瀬麻美ver）", "サンプル・見本そのもの"),
            ("動画編集ポートフォリオ｜企業VP・商品紹介・SNS広告対応｜小夏",
             "制作者のポートフォリオ"),
            ("企業VPの作り方を解説！初心者でもできる5つのコツ", "作り方の解説・教材"),
            ("映像制作フリーランスの実績集2026", "制作者のポートフォリオ"),
            ("【デモリール】CM・VP制作 showreel 2026", "制作者のポートフォリオ")):
        check(f"弾く（{_want}）: {_t[:26]}",
              bot._not_promo_reason({"title": _t}), _want)
    # 守りすぎの確認：本物の制作事例は残すこと
    for _t in ("株式会社サムシングファン 会社紹介動画",
               "トヨタ自動車 ブランドムービー「未来へ」",
               "採用動画｜1日密着 ヤマニパッケージ業務課",
               "中部電力 企業CM 2022年版",
               "【企業VP】〇〇製作所 ものづくりの現場"):
        check(f"残す: {_t[:26]}", bot._not_promo_reason({"title": _t}), None)
    # 説明文は見ないこと（本物のVPの概要欄にある「お仕事のご依頼はこちら」で
    # 誤爆するため。ボイスサンプルの判定だけは従来どおり説明文も見る）
    check("説明文の営業文では弾かない",
          bot._not_promo_reason({"title": "〇〇株式会社 企業VP",
                                 "desc": "制作：△△（お仕事のご依頼は概要欄から）"}),
          None)

    print("■ 文章の直しは『動いていない』の守り手を通さない _is_text_edit_ask")
    # 事故（2026-09-13 05:58）：「社員インタビューで、がいらない」と頼んだのに、
    # 直っていない文がそのまま返ってきた。原因は返事の作り方ではなく後処理で、
    # 「了解。直しました。」の『直しました』を _drop_false_progress が
    # 【動いていないのに作業を宣言した嘘】として落とし、答えの骨格が消えていた。
    # 文章の直しはジョブを立てずその場で返すのが正しいので、通してはいけない。
    for _t in ("社員インタビューで伝えたい、の社員インタビューで、がいらない",
               "冒頭の、お世話になります、がいらない",
               "この項目いらない", "質問をもう少し短くして",
               "見出しの表現を変えて", "この行を消して",
               "箇条書きにまとめ直して"):
        check(f"文章の直しとみなす: {_t[:26]}", bot._is_text_edit_ask(_t), True)
    # 守りすぎの確認：絵・動画への注文は従来どおり守り手を効かせること
    for _t in ("動画作って", "サムネ作り直して", "画像を生成して",
               "背景を赤にして", "切り抜きして",
               "動画の冒頭のロゴがいらない", "BGMを短くして"):
        check(f"制作の依頼のまま: {_t[:26]}", bot._is_text_edit_ask(_t), False)
    # 実際に落とされていたことの再現（守り手を通すと骨格が消える）
    _keep_busy2 = bot._busy_tasks
    try:
        bot._busy_tasks = lambda cid: []
        check("『直しました』は守り手を通すと落ちる（だから通さない）",
              bot._drop_false_progress("了解。直しました。\n\n・伝えたい方向性は？", 4242)
              != "了解。直しました。\n\n・伝えたい方向性は？", True)
    finally:
        bot._busy_tasks = _keep_busy2

    print("■ 文章の直しの時だけ、直し方の指示を渡す _text_edit_guide")
    # 事故（2026-09-13 05:58・06:06）：直せと言われた部分を残したまま、
    # 元の文をそのまま返し続けた。後処理は無実で、生成の段階で直っていなかった。
    # CLAUDE.md の作法どおり「〜の時だけ守れ」を文章でAIに守らせず、
    # 渡すかどうかをコードで決める。
    _edit_h = [("kohei", "社員インタビューで伝えたい、の社員インタビューで、がいらない")]
    check("文章の直しなら指示が入る",
          "文章の手直し" in bot._text_edit_guide(_edit_h), True)
    check("プロンプト本体にも入る",
          "文章の手直し" in bot._answer_prompt("クロード2（PM）", _edit_h), True)
    # 守りすぎの確認：関係ない発言では渡さない（毎回付くと煩わしい）
    for _t in ("動画作って", "サムネ作り直して", "企業VPでリサーチして",
               "ありがとう", "しんどい"):
        check(f"指示を渡さない: {_t}", bot._text_edit_guide([("kohei", _t)]), "")

    print("■ 貼り付けた引用で作業を決めない _instruction_part")
    # 事故（2026-09-12 09:11）：「クロード1、以下の事例を探して」の後に
    # ボットの過去の返事を丸ごと貼ったところ、引用内の「TOPPANの実績紹介動画」の
    # 『実績』で、自分のチャンネルの実績分析が始まった。
    _quoted = ("クロード1、以下の事例を探して\n\n"
               "**クロード2（PM）**: うん、あるよ。方向性としては型が参考になる。\n"
               "- **TOPPANの実績紹介動画**にあったマッチカット")
    check("引用を落として指示だけ残す",
          bot._instruction_part(_quoted), "クロード1、以下の事例を探して")
    check("引用のせいで実績分析にしない",
          bot.classify_route(_quoted), "trend")
    check("> の引用も落とす",
          bot._instruction_part("これ直して\n> 前の発言の実績の話"), "これ直して")
    # 事故（2026-09-13 06:26）：記入したヒアリングシートを貼って、最後に
    # 「クロード2へ、〜構成案を作成して」と書いたら、シート内の「動画」で
    # 【作り直し】と判定され、8月に作った動画の完成報告が返ってきた。
    # 貼り付けが先で指示が最後、という並びを拾えていなかった。
    _sheet = ("**動画制作の目的とターゲット**\n"
              "- 今回の動画で解決したい課題→採用エントリー数を上げたい\n"
              "- 活用したい既存の映像や写真素材はありますか？→去年の写真があります\n\n"
              "クロード2へ、このヒアリングシートを元に構成案を作成して")
    check("後ろに置いた宛名付きの指示を拾う",
          bot._instruction_part(_sheet),
          "クロード2へ、このヒアリングシートを元に構成案を作成して")
    check("貼り付けた資料で作り直しにしない",
          bot.classify_route(_sheet, has_last_gen=True) == "revise", False)
    # 守りすぎの確認：引用でない長文は全文のまま使うこと
    _long = "会社紹介動画を作りたいです。\n予算は50万で、尺は60秒。\n工場の雰囲気を出したい。"
    check("普通の長文は削らない", bot._instruction_part(_long), _long)

    print("■ リサーチャー（クロード1）への探し物はリサーチへ")
    for _t in ("クロード1、以下の事例を探して", "クロード1 会社紹介動画の事例探して",
               "企業VPの事例を探して", "マッチカットの事例探して",
               "リサーチャー、採用動画の参考を集めて"):
        check(f"リサーチにする: {_t[:26]}", bot.classify_route(_t), "trend")
    # 呼びかけ・枠は検索語に混ぜないこと
    for _t, _want in (("クロード1 会社紹介動画の事例探して", "会社紹介動画"),
                      ("クロード1、以下の事例を探して", ""),
                      ("企業VPの事例を探して", "企業VP"),
                      ("リサーチャー、採用動画の参考を集めて", "採用動画")):
        check(f"検索語を取り出す: {_t[:26]}", bot._trend_topic(_t), _want)

    print("■ HDDの『実績』をYouTubeチャンネルの実績分析にしない")
    # 事故（2026-09-09 17:59）：「HDDから実績抜き出して」に対して
    # 「投稿済み動画の再生数を取得し…」と答えた。HDDの話をしているのに
    # YouTubeを見に行こうとしていた。「実績」には2つの意味がある。
    # 本人の指示（2026-09-09）：既に解析してあるので、作り直さず置き場を答える。
    for _t in ("HDDから実績抜き出して", "HDDパス使って実績抜き出して",
               "外付けの制作データから実績まとめて",
               "過去の制作の実績を一覧にして"):
        check(f"HDDの実績として受ける: {_t}",
              bot.classify_route(_t), "hdd_progress")
    check("解析済みなら置き場を答える（作り直さない）",
          "解析済みです" in bot._hdd_progress_text()
          and "成果物" in bot._hdd_progress_text(), True)
    # 守りすぎの確認：本当のチャンネル実績の照会は従来どおり
    for _t in ("実績分析して", "チャンネルの実績どう？", "再生数どうなってる？",
               "実績見せて", "成績教えて", "視聴回数チェックして"):
        check(f"チャンネル実績のまま: {_t}", bot.classify_route(_t), "ch_stats")
    # 事故（2026-09-12 11:50）：営業ロープレ中の「実績数値を出すパターンで
    # 行こう」が、自分のチャンネルの再生数分析として扱われた。動画の中身の
    # 相談をしているのにYouTube APIを叩きに行っていた。
    # 「実績」「成績」は語だけでは足りない。見せて/分析して等と揃って初めて照会。
    for _t in ("実績数値を出すパターンで行こう", "実績数値をガッツリ見せる構成にしたい",
               "前職の実績は1500万", "実績紹介動画みたいな感じで",
               "松のプランは実績重視で"):
        check(f"実績の語だけでは始めない: {_t}",
              bot.classify_route(_t) == "ch_stats", False)
    ...
    # 「企業VPに関連する語句で回」という壊れた語で検索する設定になった。
    for _t in ("YouTubeリサーチを企業VPに関連する語句で回して",
               "企業VPで回して", "リサーチを採用動画で回して"):
        check(f"一回きりの依頼は設定に触らない: {_t}", bot._match_trend_genre(_t), None)
    # 毎日の設定を変える言い方は従来どおり効くこと
    for _t, _want in (("毎日のリサーチを採用動画にして", ("set", "採用動画")),
                      ("リサーチのジャンルを企業VPにして", ("set", "企業VP")),
                      ("これからリサーチを会社紹介にして", ("set", "会社紹介"))):
        check(f"設定の変更は効く: {_t}", bot._match_trend_genre(_t), _want)

    print("■ 『〇〇で回して』でリサーチが始まる（直前がリサーチの話の時だけ）")
    # 事故（2026-09-04 16:52）：ボット自身が「『企業VPで回して』ともう一度
    # 送ってください」と案内したのに、その言い方が語彙に無く会話に落ち、
    # 「まだ何も動かしていないよ」で終わった。
    check("検索語を取り出せる: 企業VPで回して", bot._trend_topic("企業VPで回して"), "企業VP")
    _keep_say2 = bot._recent_bot_say
    try:
        bot._recent_bot_say = lambda cid: (
            "まだ終わっていません。企業VPで改めて回すなら"
            "「企業VPで回して」ともう一度送ってください。")
        for _t in ("企業VPで回して", "採用動画で回して"):
            check(f"リサーチの話の直後なら始める: {_t}",
                  bot.classify_route(_t, cid=4242), "trend")
        # 守りすぎの確認：リサーチと無関係な場面の「回して」で始めない
        bot._recent_bot_say = lambda cid: "今日は暑いですね。水分とってね。"
        for _t in ("洗濯機で回して", "扇風機を回して", "企業VPで回して"):
            check(f"関係ない場面では始めない: {_t}",
                  bot.classify_route(_t, cid=4242), None)
    finally:
        bot._recent_bot_say = _keep_say2

    print("■ YouTubeリサーチの依頼を、動画の作り直しに渡さないこと")
    # 事故（2026-09-01）：「もう一回会社プロモーションでYouTubeリサーチして」の
    # 『もう一回』を作り直しが先に拾い、動画の再生成プラン（英語プロンプト・
    # クレジット消費）を出した。リサーチはクレジットを使わない別物。
    for _t in ("もう一回会社プロモーションでYouTubeリサーチして",
               "また会社紹介でYouTubeリサーチして",
               "もう一度YouTubeリサーチして"):
        check(f"作り直しに流さない: {_t[:24]!r}",
              bot.classify_route(_t, cid=1, has_last_gen=True), None)
    # 守りすぎの検出：動画の作り直しは今までどおり通ること
    for _t in ("もう一回作って", "作り直して", "明るくして"):
        check(f"動画の作り直しは通る: {_t!r}",
              bot.classify_route(_t, cid=1, has_last_gen=True), "revise")
    ...
    print("■ 画像を添付しての「動画編集して」を、既存動画の編集にしない")
    # 事故（2026-08-22）：静止画3枚を添付して「この3枚をつかって9:16で
    # 動画編集して」と頼まれたのに、添付を無視して【前に作った動画】を
    # 加工していた。「編集」の語だけでは素材か完成品か区別できないので、
    # 添付が画像かどうか（状態）で決める。
    _IMG = {"has_image_att": True, "has_attachments": True, "has_last_gen": True}
    for _t in ("この3枚をつかって9:16で動画編集して", "この3枚で動画編集して",
               "これで動画作って", "この画像を動画にして", "3枚でムービー作って"):
        check(f"素材として使う: {_t[:18]!r}",
              bot.classify_route(_t, **_IMG), "slideshow")
    # 画像を添付していなければ、従来どおり完成動画への後工程
    for _t in ("字幕つけて", "9:16にして", "15秒に縮めて"):
        check(f"動画の後工程は従来どおり: {_t!r}",
              bot.classify_route(_t, has_last_gen=True), "edit")
    check("動画を添付した編集も従来どおり",
          bot.classify_route("字幕つけて", has_video_att=True,
                             has_attachments=True), "edit")
    # 画像添付でも、絵そのものの加工は画像の経路のまま
    for _t in ("この写真の背景を消して", "2枚をいい感じに組み合わせて"):
        check(f"絵の加工は画像へ: {_t!r}", bot.classify_route(_t, **_IMG), "image")
    # 「9:16で」の指定が黙って無視されないこと
    for _t, _want in (("9:16で動画にして", (1080, 1920)), ("16:9にして", (1920, 1080)),
                      ("正方形で", (1080, 1080)), ("動画化して", None)):
        check(f"比率を読む: {_t!r}", bot._wanted_aspect(_t), _want)
    ...
    # CLAUDE.md には「実際に『直して/変えて』と命令された時だけ」とあるが、
    # プロンプトの文章では守られなかったのでコード側で止める。
    for _t in ("クロードだけで動画制作したい", "もっと速くなるといいな",
               "動画制作を効率化したい", "返事が長い気がする",
               "こういう機能あったらいいと思う", "クロードで作りたい",
               "今日つかれた", "動画作りたい", "ポートフォリオ作りたい"):
        check(f"改修を始めない: {_t!r}", bot._is_selffix_order(_t), False)
    for _t in ("返答をもっと短くして", "!trendの本数を3本に変えて",
               "きみのコードのバグを直して", "エラーを直して", "返事を短くしといて",
               "不具合直して", "確認画面を出さないようにして",
               "自動更新をオフにして", "ログの件数を増やして"):
        check(f"改修として受ける: {_t!r}", bot._is_selffix_order(_t), True)
    check("守り手が実際に使われている",
    ...
    # デバッグログの「直近のエラー」が偽物で埋まっていた（累積262件）。
    # それを見た開発側が、存在しない不具合を何日も追いかけた。
    # 書き込み先を増やしたときに隔離し忘れても、ここで気づけるようにする。
