...
/* 4K は 1080p からの引き伸ばしで情報が増えない。売り文句には置かず、FAQ にだけ
   「対応はするが中身は1080p」と正直に書く（2026-09-16） */
check('4K を売り文句にしていない',
    !/4K（高精細化）での納品にも追加費用なし/.test(idx) && !/1080p・4K で納品/.test(idx));
check('4K の中身を FAQ で正直に説明している',
    faqUi.some(u => /4K/.test(u.q) && /1080pから高精細化/.test(u.a) && /増えるわけではない/.test(u.a)));
