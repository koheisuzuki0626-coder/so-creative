   ...
   要約版には fold を1つも置かない。ここではそれを検査する */
{
    ...
    /* 初入金は 9/22 に着手金を外したので「約1ヶ月」→「約2ヶ月」
       （納品してから期日30日ぶん待つ） */
    check('未開業・クレジット残・事例0本・初入金までが出ている',
        now.join('/') === '未開業/246.84/0本/約2ヶ月', now.join('/'));
    ...
    check('要約版に収支の推移の要点がある',
        /収支の推移/.test(t) && /赤字になるのは開業した月だけ/.test(t));
    check('要約版に月ごとの表が載っている',
        /M\+0/.test(t) && /M\+11/.test(t) && /454\.1万/.test(t));
    check('推移の前提を書いている',
        /支出 月13\.8万/.test(t) && /全額を納品後に請求/.test(t));
    /* 9/22 に着手金を外した。表の数字が古いままだと判断を誤るので、
       開業月の底（−13.8万）と、手元が戻る月（M+2＝38.1万）を見る */
    check('着手金なしの数字に直っている',
        /−13\.8万/.test(t) && /38\.1万/.test(t) && !/484\.1万/.test(t));
