   ...
   要約版には fold を1つも置かない。ここではそれを検査する */
{
    ...
    /* 金額の推移。開業日が未定なので M+0 からの相対で置く。
       要約版には要点だけ、月ごとの表は record.html（9/21） */
    check('要約版に金額の推移の要点がある',
        /金額の推移/.test(t) && /赤字は開業した月だけ/.test(t));
    ...
    check('支払条件は残っている（決定事項）',
        /着手金50%/.test(t) && /納品日から30日以内/.test(t));
    check('着手金を値引きの対象にしないと書いている', /着手金を値引きの対象にしない/.test(t));
