...
読みは「まもりば」。若葉・青葉と同じ連濁で、こちらが正。
サイトの本文や alt もこの読みで書く。
...
# 刷り面の高さは、幾何どおりだと文字が縦長に見えるので、5%ほど詰めてある。

# チューブの輪郭（2048px の写真から実測）。rot はチューブを縦に起こす角度。
# prof は (y, 見かけの半径, 中心x) の実測列。間は線形で補う。
GEOM = {
    # 立っているカット
    "p1": dict(rot=0.0, y0=607.0, y1=1388.0, fill=0.80,
               prof=[(430.0, 236.0, 1017.0),
                     (1470.0, 151.0, 1029.0)]),
    # 寝かせたカット（チューブを縦に起こしてから刷る）
    "p2": dict(rot=-92.3, y0=749.0, y1=1391.0, fill=0.95,
