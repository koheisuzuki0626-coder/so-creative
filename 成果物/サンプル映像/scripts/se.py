...
if __name__ == "__main__":
    ...
    for name, b, s in (("02", b02, s02), ("04a", b04, s04a), ("04b", b04, s04b),
        ...
        # 他の映像は BGM が主で SE はその下に敷く層だが、07 は SE だけが鳴るので
        # 同じ総量にすると環境音そのものが大きく聞こえる。
        # 納品済みの5本で測ると 07 だけ -14.7dB（他は -16.4〜-18.5dB）だったので、
        # いちばん静かな 01 会社紹介（-18.5dB）より下に収まるところまで落とす。
        # トランジェントが多く AAC 変換でピークが張り付くのは変わらないので drive は据え置き
        m = mix(b, s, se_level=1.2 if name == "07" else 0.9,
                peak=0.37 if name == "07" else 0.9,
