...
if __name__ == "__main__":
    ...
    # 締めはロゴ札をやめて、食卓の1カット5秒。後半で抜いたパッケージが
    # 右下に乗る。名前は袋に刷ってあるので、別にロゴを出すと重複する
    if wanted("04a_taste_15s"):
        build("04a_taste_15s",
              [(f"{GEN}/04_v1.mp4", 0.30, 1.6),        # 凍ったまま並べる
               ...
               # 締めは食卓の1カットで5秒。12.5秒（このカットの2.5秒）から
               # 抜いたパッケージが右下に乗る。BGM の旋律が F#5 に上がる
               # ところ。階調は package.on_table 側で当てているので素通し
               (f"{GEN}/04_table_pack.mp4", 0.00, 5.0, 1.0, "")],
