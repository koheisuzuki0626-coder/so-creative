...
if __name__ == "__main__":
    ...
    # 並べる／皿に返した羽根を足して8カットにしている。
    # 締めはロゴ札をやめて、食卓 2.5秒 → 画面いっぱいの商品カット 2.5秒。
    # 食卓の右下にパッケージを小さく乗せる形も試したが、出し方をどう作っても
    # 貼り付けた感じが残った。画をまるごと商品に渡すほうが強い。
    # 名前は袋に刷ってあるので、別にロゴを出すと重複する
    if wanted("04a_taste_15s"):
        build("04a_taste_15s",
              [(f"{GEN}/04_v1.mp4", 0.30, 1.6),        # 凍ったまま並べる
               ...
               # 食卓で2.5秒みせてから、画面いっぱいの商品カットに切り替える。
               # 切り替えは BGM の旋律が F#5 に上がる 12.5秒に合わせた。
               # パッケージは刷った静止画なので、持ち上げはかけない
               (f"{GEN}/04_c3_v3m.mp4", 0.04, 2.5),
               (f"{GEN}/04_pack.mp4", 0.00, 2.5, 1.0, "")],
